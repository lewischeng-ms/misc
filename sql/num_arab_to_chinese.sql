...
begin
	for i in 1..len loop
		...
		if num <> 0 then --非'0'时处理：
			if s_num = 0 then
				...
				srmb := srmb || case --否则：
					...
					when e_num < 12 then --否则(多个'0'处理)
						case
							when s_num < 7 then '' --万以内..
							when s_num < 11 then
								case
									when e_num < 8 and s_num < 10 then '万'
								end
							when s_num < 15 then
								case
									when e_num < 12 then '亿'
								end
							else '万亿'
						end
					when e_num < 16 and s_num > 14 then '万'
				end || case
					when s_num > 3 and e_num < 3 then '圆零'
					when e_num = 3 then '圆'
					when e_num not in (7, 11, 15) or s_num - e_num > 2 then '零'
				end;
			...
			if s_num = 0 then --当s_num = 0时'0'串起始，
				s_num := len - i + 1; --记录开始
				e_num := s_num; --各结整位置。
			else
	...
	if s_num <> 0 then --此时以'0'结尾
		srmb := srmb ||	case
			when s_num = len then '零圆整' --全'0'串，加...
